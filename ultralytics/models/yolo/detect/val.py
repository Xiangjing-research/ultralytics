...
from ultralytics.utils import DEFAULT_CFG, LOGGER, ops, colorstr, emojis, callbacks, TQDM
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            if isinstance(self.args.data, str) and self.args.data.split('.')[-1] in ('yaml', 'yml'):
                self.data = check_det_rgb_ir_dataset(self.args.data)
            # elif self.args.task == 'classify':
            #     self.data = check_det_rgb_ir_dataset(self.args.data, split=self.args.split)
            else:
        ...
        for batch_i, batch in enumerate(bar):
            ...
            # Inference
            with dt[1]:
                rgn_img, ir_img = torch.split(batch['img'], split_size_or_sections=3, dim=1)
                preds = model(rgn_img, augment=augment)
...
if __name__ == '__main__':
    # val(use_python=True)
    validator = MultiModalDetectionValidator(args=DEFAULT_CFG)
